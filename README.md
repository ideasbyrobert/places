...
GridWindowManager explains why Window Control permission is required before opening System Settings. In Privacy & Security, choose Device Control and Data Access on newer macOS releases or Accessibility on earlier releases. Enable `GridWindowManager.app` itself, not an app whose name ends in `Tests-Runner`, then return to GridWindowManager. No Screen Recording or Input Monitoring permission is requested. Terminal Automation is separate and is requested only when you explicitly arrange Terminal windows or choose Request in Settings.
...
Show Desktop uses public macOS application hide and unhide operations. It records only the visible regular applications it actually hid, excludes Finder and GridWindowManager, and restores only that recorded set. A partial hide is reverted immediately, and a partial restore remains retryable. It does not create, reorder, or automate virtual Spaces.
...
The permission identities are not interchangeable. Normal use and `--live-app-accessibility` require `GridWindowManager.app`. Direct `--live-accessibility` and `--live-terminal` integration tests require `GridWindowManagerTests-Runner.app`. Do not manually add `GridWindowManagerUITests-Runner.app`; it is Xcode's UI-test host, not the window manager.
