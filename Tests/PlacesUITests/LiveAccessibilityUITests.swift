...
final class LiveAccessibilityUITests: XCTestCase
{
    func testRealAppArrangesAndRestoresFixtureWindow() throws
    {
        ...
        let fixture = XCUIApplication(
            bundleIdentifier: "com.ideasbyrobert.Places.WindowFixtureApp"
        )
        ...
        [
            "--ui-testing-live-target-bundle-identifier=com.ideasbyrobert.Places.WindowFixtureApp",
            "--ui-testing-live-arrange-and-restore"
        ]
        manager.launch()
        if manager.staticTexts["Allow Window Control"].waitForExistence(timeout: 1)
        {
            throw XCTSkip("Places.app does not have Window Control permission.")
        }
